...
class File(
    ...
    init {
        val resources = Config.activity.resources
...
class Config {
    companion object {
        ...
        val sounds: Array<Sound> = Array()
        var n_sounds: Int = 0

        var time_watch: Long = 0

        var ready: Boolean = false

        fun set_activity(activity: Activity) {
...
open class Image {
    ...
    fun on_window(
        ...
        window.drawing.drawBitmap(drawable, x_image, y_image, Config.paint)
    }
...
class Window(
    ...
    fun draw_rectangle(
        ...
        val x_min = Math.to_float(left)
        val x_max = Math.to_float(right)
        val y_min = Math.to_float(top)
        val y_max = Math.to_float(bottom)
        drawing.drawRect(x_min, y_min, x_max, y_max, Config.paint)
    }
}
...
class Sound(
    ...
    init {
        Config.sounds.add(this)
    }

    fun load() {
        val context = Config.activity as Context
        listenable = Config.sound_pool.load(context, id, 1)
    }
    ...
    fun play(is_loop: Boolean) {
        ...
        val sound_pool = Config.sound_pool
        id_sound = sound_pool.play(listenable, volume, volume, 1, loop, 1F)
    }

    fun stop() {
        if (id_sound != -1) {
            Config.sound_pool.stop(id_sound)
            id_sound = -1
        }
    }
    ...
    fun set_volume(volume_init: Double) {
        ...
        if (id_sound != -1) {
            val volume = Math.to_float(volume_current)
            Config.sound_pool.setVolume(id_sound, volume, volume)
        }
    }
}
