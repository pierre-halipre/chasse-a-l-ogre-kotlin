...
class Text {
    ...
    fun setCustomPause(
        ...
        val millisecondsFinal: Int

        if (milliseconds >= 3600000) {
            millisecondsFinal = 3599999
        } else {
            millisecondsFinal = milliseconds
        }

        val minutes = Math.floor(millisecondsFinal / 60000)
        val minutesTen = Math.floor(minutes / 10)
        val minutesUnit = minutes % 10
        val minutesSeparator = 10
        val seconds = Math.floor((millisecondsFinal % 60000) / 1000)
        val secondsTen = Math.floor(seconds / 10)
        val secondsUnit = seconds % 10
        val secondsSeparator = 11
        val centiseconds = Math.floor((millisecondsFinal % 1000) / 10)
    ...
    fun setCustomEnd(
        ...
        val thousand = Math.floor(scoreFinal / 1000)
        val hundred = Math.floor((scoreFinal % 1000) / 100)
        val ten = Math.floor((scoreFinal % 100) / 10)
        val unit = score % 10
        val space = 12
